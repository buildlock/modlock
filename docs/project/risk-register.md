...
| R-021 | BuildLock and Modlock implement competing mod registries, artifact IDs, or creator identities | 4 | 5 | 20 | Resolve OQ9/OQ10 across Deadlock-Infra/BuildLock/Modlock; one canonical mod authority; contract compatibility fixtures; no private-table coupling | Either repo starts schema/runtime work before shared boundary acceptance | Portfolio architecture/product |
| R-022 | Dirty Phase 0 candidate drifts from current main or is mechanically merged over canonical docs/contracts | 4 | 4 | 16 | Read-only independent review; source-manifest pin; keep/rework/discard verdict; path-by-path collision manifest; one writer; fresh CI on integrated current-main PR | Rebase/copy starts before review or either tree changes without manifest refresh | Engineering/release |
