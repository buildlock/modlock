...
import { currentCatalogProfiles } from "./catalog-refresh";
...
async function sharedCatalog(): Promise<Catalog | null> {
  ...
  const result = await getDatabase().query<{ document: Catalog }>(
...
export async function getCatalog(): Promise<Catalog | null> {
  try {
    ...
    const current = currentCatalogProfiles(data);
    if (config.enabled) {
      const hidden = await restrictions();
      return {
        ...current,
        mods: current.mods.filter((mod) => !hidden.has(mod.key)),
      };
    }
    return current;
  } catch (error) {
